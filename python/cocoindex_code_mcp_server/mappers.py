...
SOURCE_CONFIG = {
    ...
    # Fallback exclusion list — only applied when no .gitignore files are found in the
    # scan tree.  When .gitignore files are present they replace this list entirely,
    # because they are the authoritative source of exclusions for that project.
    # Each directory-name pattern appears twice: once to match the directory node itself
    # and once with /** to match all files inside it.
    "excluded_patterns": [
        "**/.*",
        "**/.*/**",
        ...
        "**/.vscode",
        "**/.vscode/**",
        # Rust-specific subdirectories (subset of target/ but listed explicitly)
        "**/target/debug",
        "**/target/debug/**",
        "**/target/release",
        "**/target/release/**",
