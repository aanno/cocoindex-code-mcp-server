...
class TestRustHaskellErrorHandling:
    ...
    def test_haskell_buggy_code_high_errors(self):
        ...
        assert result.error_stats().error_count() >= ERROR_FALLBACK_THRESHOLD, f"Expected at least {ERROR_FALLBACK_THRESHOLD} errors"
    ...
    def test_haskell_minor_errors_ast_recovery(self):
        ...
        assert 1 <= error_count < ERROR_FALLBACK_THRESHOLD, f"Expected 1-{ERROR_FALLBACK_THRESHOLD-1} errors, got {error_count}"
        assert not result.error_stats().should_fallback(), "Should not trigger fallback"
        assert result.chunking_method() == "ast_with_errors", f"Expected ast_with_errors, got {result.chunking_method()}"
...
class TestPythonTreeSitterErrorHandling:
    ...
    def test_python_error_stats_initialization(self):
        ...
        try:
            from cocoindex_code_mcp_server.ast_visitor import GenericMetadataVisitor, ErrorStats
        except ImportError:
...
'''
    ...
    def test_haskell_visitor_minor_errors(self):
        ...
        assert 1 <= error_count < ERROR_FALLBACK_THRESHOLD, f"Should have 1-{ERROR_FALLBACK_THRESHOLD-1} errors, got {error_count}"
...
class TestPythonErrorExamples:
    ...
    def test_python_minor_errors_detection(self):
        ...
        assert analysis_method in ['tree_sitter', 'python_ast', 'enhanced_regex', 'basic_text'], f"Unexpected analysis method: {analysis_method}"
