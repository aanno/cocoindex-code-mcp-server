...
try:
    from hybrid_search import HybridSearchEngine, format_results_as_json, format_results_readable
except ImportError:
    print("⚠️  Warning: Could not import hybrid_search module. These are integration tests that require the full application setup.")
...
def run_manual_metadata_search():
    ...
    if results:
        ...
        print(f"\n📊 Metadata Summary:")
        print(f"  Functions found: {len(functions_found)} ({', '.join(functions_found[:5])}{'...' if len(functions_found) > 5 else ''})")
        print(f"  Classes found: {len(classes_found)} ({', '.join(classes_found[:5])}{'...' if len(classes_found) > 5 else ''})")
        print(f"  Unique imports: {len(imports_found)} ({', '.join(list(imports_found)[:5])}{'...' if len(imports_found) > 5 else ''})")
